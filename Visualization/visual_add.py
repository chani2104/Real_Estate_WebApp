...
# 데이터 불러오기
try:
    df = pd.read_csv('../data/region_rent_summary.csv')
except:
    df = pd.read_csv('data/region_rent_summary.csv')

# --- 데이터 전처리 및 파생변수 생성 ---
# 6번 가성비 지표: 전세 데이터가 있는 경우만 계산 (분모가 0인 경우 방지)
df['전세_면적당보증금'] = df.apply(lambda x: x['전세_평균보증금'] / x['전세_평균면적'] if x['전세_평균면적'] > 0 else 0, axis=1)

# 7번 월세 효율 지표: 월세 데이터가 있는 경우만 계산
df['월세_면적당월세액'] = df.apply(lambda x: x['월세_평균월세'] / x['월세_평균면적'] if x['월세_평균면적'] > 0 else 0, axis=1)

# 시각화 가독성을 위해 상위 지역 추출 시 사용할 데이터 정렬
df_top20_total = df.nlargest(10, '전체_거래건수')
df_top20_rent = df.sort_values('월세_평균월세', ascending=False).head(20)
...
df_area.plot(x='sggNm', y=['전세_평균면적', '월세_평균면적'], kind='bar', figsize=(12, 6))
...
plt.figure(figsize=(10, 6))
sns.regplot(data=df[df['월세_평균월세'] > 0], x='월세_평균면적', y='월세_평균월세', scatter_kws={'alpha':0.5}, line_kws={'color':'red'})
